...
- Expose the PR23/PR28 local verify-loop contract through `aiweb verify-loop` / `웹빌더 verify-loop`: it connects build → preview → Playwright/accessibility/Lighthouse/screenshot QA → visual critique → repair or visual-polish task → approved agent-run cycles, supports `--agent codex|openmanus`, caps `--max-cycles` at 10, supports no-write/no-process `--dry-run`, requires `--approved` for real local execution, records `.ai-web/runs/verify-loop-<timestamp>/verify-loop.json` plus per-cycle evidence and deploy provenance, blocks missing dependencies with a `setup --install --approved` next action, and never installs packages, deploys, calls provider CLIs, or reads `.env` / `.env.*`.
