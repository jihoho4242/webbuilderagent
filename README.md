...
`verify-loop` remains as an engine-run compatibility shim only:

```bash
./bin/aiweb --path ~/Desktop/aiweb-premium-service-site verify-loop --max-cycles 3 --dry-run --json
./bin/aiweb --path ~/Desktop/aiweb-premium-service-site verify-loop --max-cycles 3 --approval-hash HASH --approved --json
./bin/aiweb --path ~/Desktop/aiweb-premium-service-site verify-loop --max-cycles 3 --agent openmanus --dry-run --json
./bin/aiweb --path ~/Desktop/aiweb-premium-service-site verify-loop --max-cycles 3 --agent openmanus --sandbox docker --approval-hash HASH --approved --json
?밸퉴??--path ~/Desktop/aiweb-premium-service-site verify-loop --max-cycles 3 --dry-run
?밸퉴??--path ~/Desktop/aiweb-premium-service-site verify-loop --max-cycles 3 --approval-hash HASH --approved
```

`verify-loop --dry-run` writes nothing and launches no processes; it reports the canonical `engine-run` approval hash and planned engine-run artifacts. The former closed script of build, preview, QA, screenshot, visual critique, repair/visual-polish, and agent-run cycles has been removed. `--max-cycles` accepts 1 through 10. `--agent codex|openmanus` selects the delegated engine-run adapter; approved OpenManus execution still requires `--sandbox docker|podman`. Real local work requires `--approved --approval-hash HASH` and is delegated to `engine-run` instead of writing `.ai-web/runs/verify-loop-*` cycle evidence; verify-loop evidence cannot unlock deploy. It never installs packages, deploys, pushes, calls provider CLIs, or reads/prints `.env` / `.env.*`.
...
./bin/aiweb verify-loop --max-cycles 3 --approval-hash HASH --approved
./bin/aiweb verify-loop --max-cycles 3 --agent openmanus --sandbox docker --approval-hash HASH --approved
