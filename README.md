...
- Expose `aiweb verify-loop` / `?밸퉴??verify-loop` as a legacy compatibility shim only: the former hardcoded build ??preview ??QA ??visual critique ??repair/visual-polish ??agent-run script has been removed. `--dry-run` writes nothing and returns the canonical `engine-run` approval hash/plan; real local execution must provide `--approval-hash HASH --approved` and is delegated to `engine-run` policy, sandbox, graph, and copy-back gates.
...
- Expose the PR19/PR27/PR28 GitHub sync and deploy surfaces through `aiweb github-sync`, `aiweb deploy-plan`, and `aiweb deploy --target cloudflare-pages|vercel --dry-run|--approved` / matching `?밸퉴?? commands: dry-runs remain no-write/no-process local planning. Approved deploy is fail-closed and must not be unlocked by the removed verify-loop script-runner; engine-run release evidence migration is a remaining release-gate task before real provider CLI use. They never run `git push`, install/build/preview, or read `.env` / `.env.*`.
...
- Expose the local backend bridge through `aiweb daemon` / `aiweb backend`: it binds only to localhost-class hosts by default, allows only local browser origins, requires `X-Aiweb-Token` for every `/api/*` request, exposes JSON endpoints for the future web Workbench, invokes this repository's `bin/aiweb` by absolute path instead of shell interpolation, keeps approved source-patch/setup/verify-loop/agent execution behind `X-Aiweb-Approval-Token` plus hash-bound `--approval-hash` / `--approval-request` command args, and blocks raw shell, frontend-supplied backend flags, missing project paths, unsafe deploy, and `.env` / `.env.*` paths.
...
?밸퉴??--path ~/Desktop/aiweb-premium-service-site deploy --target vercel --approved
```
...
./bin/aiweb --path ~/Desktop/aiweb-premium-service-site agent-run --task latest --agent codex --approval-hash HASH --approved --json
./bin/aiweb --path ~/Desktop/aiweb-premium-service-site agent-run --task latest --agent openmanus --dry-run --json
./bin/aiweb --path ~/Desktop/aiweb-premium-service-site agent-run --task latest --agent openmanus --sandbox docker --approval-hash HASH --approved --json
?밸퉴??--path ~/Desktop/aiweb-premium-service-site agent-run --task latest --agent codex --dry-run
?밸퉴??--path ~/Desktop/aiweb-premium-service-site agent-run --task latest --agent codex --approval-hash HASH --approved
```

`agent-run --dry-run` is a no-write / no-process preflight: it reports the planned `.ai-web/runs/agent-run-<timestamp>/agent-run.json`, `stdout.log`, `stderr.log`, and `.ai-web/diffs/agent-run-<timestamp>.patch` paths without executing a local agent. A real `agent-run` requires the `approval_hash` emitted by `agent-run --dry-run` plus `--approved`; omitting either blocks execution with approval-required semantics and writes nothing. `--agent codex` keeps the original local source-patch adapter. `--agent openmanus` uses an aiweb-managed Docker/Podman sandbox plus JSON file contract, clean environment, no-network/no-MCP guard env, workspace-scoped `.ai-web/tmp/openmanus/<run-id>/` staging directory, hunk/workspace validation, OpenManus result JSON validation, and copy-back of allowed source files only after staged diff validation. Approved OpenManus execution is blocked unless the matching dry-run `approval_hash` and `--sandbox docker|podman` are supplied and aiweb verifies the generated command contains `--network none`, read-only root, dropped capabilities, no-new-privileges, resource limits, restricted tmpfs, and exactly one writable mount for the staging workspace. The command is limited to `.ai-web/tasks/*.md` task packets with `Goal`, `Inputs`, `Constraints`, and machine constraints (`shell_allowed: false`, `network_allowed: false`, `env_access_allowed: false`, `allowed_source_paths:`). It reads only task/design/component-map/selected-design/source context that is already allowed by the task packet, refuses `.env` / `.env.*`, secret-looking paths, symlink/hardlink source paths, and shell/network/package/deploy requests, rejects approved runs that mutate files outside the allowed source list, redacts secret-looking process output, captures stdout/stderr and a source diff patch when it runs, and records the latest run metadata in `.ai-web` safe state. `agent-run` itself does not run build/preview/QA/deploy/provider CLIs and does not treat `--force` as approval.
...
Use `agent ... --dry-run` or `engine-run ... --dry-run` as the primary natural-language planning surface. `verify-loop` remains only as a legacy compatibility shim: it writes nothing in dry-run, launches no processes, and reports the canonical `engine-run` approval hash and planned engine-run artifacts. The former closed script of build, preview, QA, screenshot, visual critique, repair/visual-polish, and agent-run cycles has been removed. Real local work requires a matching `--approval-hash HASH --approved` on the canonical `engine-run` path; verify-loop must not write `.ai-web/runs/verify-loop-*` cycle evidence and cannot unlock deploy. It never installs packages, deploys, pushes, calls provider CLIs, or reads/prints `.env` / `.env.*`.
...
./bin/aiweb agent-run --task latest --agent codex --approval-hash HASH --approved
./bin/aiweb agent-run --task latest --agent openmanus --sandbox docker --approval-hash HASH --approved
./bin/aiweb engine-run --agent codex --mode agentic_local --max-cycles 3 --dry-run
