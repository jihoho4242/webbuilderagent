...
- Expose the PR22 local source-patch agent-run contract through `aiweb agent-run` / `?밸퉴??agent-run`: it requires an explicit task and `--agent codex|openmanus`, supports `--dry-run` without writes or processes, requires the dry-run `approval_hash` plus `--approved` for real execution, captures stdout/stderr/diff evidence under `.ai-web`, rejects `.env` / `.env.*` access, and does not run build/preview/QA/deploy, provider CLI, or treat `--force` as approval. OpenManus approved execution requires the dry-run `approval_hash` plus `--sandbox docker|podman`; aiweb constructs and validates a no-network, read-only-root, dropped-capability container command with only the staging workspace mounted writable before any copy-back can occur.
- Expose `aiweb verify-loop` / `?????verify-loop` as a read-only migration shim only: the former hardcoded build ??preview ??QA ??visual critique ??repair/visual-polish ??agent-run script has been removed. It now reports the canonical `engine-run` dry-run approval hash/plan and blocks local execution even if `--approval-hash HASH --approved` is supplied; real local execution must use `aiweb engine-run` directly so the agent runtime, policy, sandbox, graph, and copy-back gates own the run.
...
15. Run `setup --install --dry-run` to inspect the planned dependency install and approval hash, then `setup --install --approval-hash HASH --approved` only when you explicitly approve that exact local package-install capability.
...
./bin/aiweb --path ~/Desktop/aiweb-premium-service-site agent "run approved local QA gates" --mode supervised --approval-hash HASH --approved --json
...
./bin/aiweb --path ~/Desktop/aiweb-premium-service-site workbench --serve --approval-hash HASH --approved
...
Use `agent ... --dry-run` or `engine-run ... --dry-run` as the primary natural-language planning surface. `verify-loop` remains only as a read-only migration shim: it launches no processes, reports the canonical `engine-run` approval hash and planned engine-run artifacts, and blocks execution even if `--approval-hash HASH --approved` is supplied. The former closed script of build, preview, QA, screenshot, visual critique, repair/visual-polish, and agent-run cycles has been removed. Real local work requires moving to the canonical `engine-run` path with its matching `--approval-hash HASH --approved`; verify-loop must not write `.ai-web/runs/verify-loop-*` cycle evidence and cannot unlock deploy. It never installs packages, deploys, pushes, calls provider CLIs, or reads/prints `.env` / `.env.*`.
