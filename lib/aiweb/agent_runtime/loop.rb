  ...
  module AgentRuntime
    class Loop
      ...
      def run(goal:, mode:, profile:, max_steps:, approved: false, dry_run:)
        ...
        report = final_report(session: session, observation: observation, plan: plan, tool_results: tool_results, verification: verification, reflection: reflection, contract: contract, dry_run: dry_run)
        changed_files = []
        unless dry_run
          changed_files = write_artifacts(session, observation, plan, tool_results, verification, reflection, report, contract)
          changed_files.concat(write_state_pointer(session, report, status))
        end
        {
          "schema_version" => 1,
      ...
      def final_report(session:, observation:, plan:, tool_results:, verification:, reflection:, contract:, dry_run:)
        not_tested = not_tested_items(session: session, contract: contract, tool_results: tool_results, dry_run: dry_run)
        {
          ...
          "patchManifest" => source_patch_manifest(session: session, observation: observation, plan: plan, contract: contract),
          "safety" => {
            "dot_env_read" => false,
      ...
      def write_artifacts(session, observation, plan, tool_results, verification, reflection, report, contract)
      ...
      def source_patch_manifest(session:, observation:, plan:, contract:)
        source_supported = contract&.supports?(:source_patch) == true
        guard_result = SourcePatchGuard.new.validate(manifest: {
          "allowed_source_paths" => %w[src public package.json astro.config.mjs next.config.mjs tsconfig.json tailwind.config.mjs test docs],
          "max_changed_files" => 20,
          "max_patch_bytes" => 200_000
        }, changed_files: [], patch_bytes: 0)
        {
          "schema_version" => 1,
