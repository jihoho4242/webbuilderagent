    ...
    def verify_loop(max_cycles: 3, approved: false, dry_run: false, force: false)
      ...
      cycle_limit.times do |index|
        ...
        unless verify_loop_step_passed?(preview_result, "preview")
          latest_blocker = verify_loop_step_blocker("preview", preview_result)
          final_status = "blocked"
          stop_reason = "preview_blocked"
          cycle["status"] = "blocked"
          cycle["blocking_issues"] << latest_blocker
          break
        end
        ...
        qa_results = []
        qa_results << verify_loop_record_step(cycle, "qa-playwright") { qa_playwright(url: preview_url, task_id: "verify-loop-cycle-#{cycle_number}-playwright", force: force, dry_run: false) }
        ...
        if blocked_qa
          latest_blocker = verify_loop_step_blocker("qa", blocked_qa)
          final_status = "blocked"
          stop_reason = "qa_blocked"
          cycle["status"] = "blocked"
          cycle["blocking_issues"] << latest_blocker
          break
        end
        ...
        if failed_qa
          ...
          unless verify_loop_step_passed?(repair_result, "repair")
            latest_blocker = verify_loop_step_blocker("repair", repair_result)
            final_status = verify_loop_step_status(repair_result) == "blocked" && latest_blocker.to_s.match?(/budget|cycle|cap/i) ? "max_cycles" : "blocked"
            stop_reason = final_status == "max_cycles" ? "repair_budget" : "repair_blocked"
            cycle["status"] = final_status == "max_cycles" ? "max_cycles" : "blocked"
            cycle["blocking_issues"] << latest_blocker
            break
          end
          ...
          unless verify_loop_step_passed?(agent_result, "agent-run")
            latest_blocker = verify_loop_step_blocker("agent-run", agent_result)
            final_status = "agent_run_failed"
            stop_reason = "agent_run_failed"
            cycle["status"] = "agent_run_failed"
            cycle["blocking_issues"] << latest_blocker
            break
          end

          if cycle_number == cycle_limit
            latest_blocker = "verify-loop reached max cycles with unresolved QA failures"
            final_status = "max_cycles"
            stop_reason = "max_cycles"
            cycle["status"] = "max_cycles"
            cycle["blocking_issues"] << latest_blocker
            break
          end
        ...
        unless verify_loop_step_passed?(polish_result, "visual-polish")
          latest_blocker = verify_loop_step_blocker("visual-polish", polish_result)
          final_status = verify_loop_step_status(polish_result) == "blocked" && latest_blocker.to_s.match?(/budget|cycle|cap/i) ? "max_cycles" : "blocked"
          stop_reason = final_status == "max_cycles" ? "visual_polish_budget" : "visual_polish_blocked"
          cycle["status"] = final_status == "max_cycles" ? "max_cycles" : "blocked"
          cycle["blocking_issues"] << latest_blocker
          break
        end
        ...
        if cycle_number == cycle_limit
          latest_blocker = "verify-loop reached max cycles with unresolved visual critique failures"
          final_status = "max_cycles"
          stop_reason = "max_cycles"
          cycle["status"] = "max_cycles"
          cycle["blocking_issues"] << latest_blocker
