    ...
    module HelpText
      TEXT = <<~HELP
        ...
        Phase-sensitive commands are guarded:
          ...
          agent: goal facade over canonical engine-run durable runtime; AgentRuntime remains compatibility evidence only, plan-only/dry-run writes no source, all side-effect actions pass DecisionPacket -> PolicyKernel -> ToolGateway, supervised/autonomous-local cannot bypass L3+ approval, and source mutation remains manifest/verifier gated
