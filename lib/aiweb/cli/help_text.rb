...
module Aiweb
  ...
  engine-scheduler daemon [--run-id latest|ID] [--max-ticks N] [--interval-seconds N] [--workers N] [--approval-hash HASH] [--approved] [--execute]
          engine-scheduler supervisor [--run-id latest|ID] [--max-ticks 0] [--interval-seconds N] [--workers N]
          engine-scheduler monitor [--dry-run]
          mcp-broker call --server lazyweb --tool lazyweb_health|lazyweb_search [--query QUERY] [--limit N] [--endpoint URL] [--approval-hash HASH] [--approved]
          mcp-broker call --server project_files --tool project_file_metadata|project_file_list|project_file_excerpt|project_file_search --query RELATIVE_PATH_OR_SEARCH [--limit N] [--approval-hash HASH] [--approved]
          ...
          eval-baseline import --path .ai-web/eval/candidate-human-baselines.json --approval-hash HASH --approved
          design-brief [--force]
  ...
  engine-scheduler: project-local durable graph scheduler service surface; status is read-only, tick records .ai-web/runs/<run-id>/artifacts/scheduler-service.json plus .ai-web/scheduler/ledger.jsonl, daemon records .ai-web/scheduler/daemon.json plus heartbeat/worker-pool artifacts for a foreground loop, supervisor records .ai-web/scheduler/supervisor.json with external service-unit/runbook templates but does not install OS services, monitor records .ai-web/scheduler/monitor.json health evidence over heartbeat/leases/queue/worker-pool artifacts, and --execute resumes through the explicit engine-run bridge only with --approved plus the matching resume --approval-hash
          mcp-broker: approved implementation-worker MCP connector broker for Lazyweb health/search plus project_files metadata/list/bounded-excerpt/bounded-literal-search only; --dry-run writes nothing and returns an approval_hash bound to the broker capability envelope, real approved calls require the matching --approval-hash HASH plus --approved, unapproved calls write deny/block audit evidence only, unknown connectors record a missing-driver fail-closed contract, approved Lazyweb calls require configured credentials, approved project_files calls use no credentials/network and return metadata or safe bounded excerpts/search matches only, redact endpoint/token/output, and record .ai-web/runs/mcp-broker-*/mcp-broker.json plus side-effect-broker.jsonl
          eval-baseline: creates human review packs and validates/imports a human-calibrated eval baseline corpus under .ai-web/eval; review-pack writes placeholders only, validate records redacted validation evidence only, import dry-run returns an approval_hash bound to the exact corpus/import capability, real import requires matching --approval-hash HASH plus --approved, rejects .env/.env.* paths, raw secrets, invalid 0..100 scores, and non-human-calibrated corpora, and never fabricates reviewer evidence
