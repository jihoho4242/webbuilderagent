...
- `ruby -Ilib -e "side_effect_surface_audit engine-run sandbox attestation classification"` ? PASS: coverage=classified, unclassified=0, entry_count=28, removed sandbox_runtime_attestation_exception; remaining `sandbox_process.rb` Open3.capture3 is brokered_engine_run_capture_command only
...
- `ruby -Ilib -e "side_effect_surface_audit engine-run worker classification"` ? PASS: coverage=classified, unclassified=0, entry_count=25, no `engine_run.rb` direct Open3 entries remain; `brokered_engine_run_capture_command` only remains in `sandbox_process.rb` `engine_run_capture_command`
