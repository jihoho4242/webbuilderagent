...
- Converted `aiweb verify-loop` from a read-only `engine-run` migration shim into a removed-command tombstone with `fixed_pipeline_present=false`, `engine_run_delegation_present=false`, `execution_allowed=false`, no approval hash, no bespoke steps, no verify-loop cycle evidence writer, and no approved execution delegation.
...
- Removed the backend generic command bridge's approved-without-hash shortcut: approved generic `agent`, `agent-run`, `setup`, and direct bridge `engine-run` requests must now include `--approval-hash HASH` or `--approval-request HASH` before the bridge forwards `--approved`; verify-loop requests are removed from the backend bridge command allowlist before `bin/aiweb` launches.
...
- Removed approved-only next-action/live guidance shortcuts: agent facade, engine-run resume guidance, eval-baseline import guidance, and human-review-pack reviewer guidance now require dry-run review plus a matching `--approval-hash HASH --approved`; verify-loop no longer has an approval-capable execution path.
...
- Demoted webbuilder/README verify-loop-first examples: launcher help and README now steer first to natural-language `agent ... --dry-run` and canonical `engine-run ... --dry-run`; verify-loop remains removed-command tombstone prose only, not the primary approved execution example.
...
- Removed verify-loop approved execution delegation: `verify-loop --approval-hash HASH --approved` now blocks as removed-command tombstone guidance, backend bridge verify-loop requests are removed from the allowlist before `bin/aiweb`, and `configs/tool_registry.yaml` marks verify_loop as a removed legacy script-runner tombstone with `execution_available=false`.
...
- `ruby -Itest test/test_contracts.rb -n /side_effect_surface_audit/` after verify-loop backend tombstone ? PASS: 5 runs, 82 assertions, 0 failures, 0 errors
- `ruby -Itest test/test_aiweb_cli.rb -n /verify_loop|verify-loop/` after verify-loop removed tombstone ? PASS: 11 runs, 301 assertions, 0 failures, 0 errors
...
- `ruby -Itest test/test_aiweb_cli.rb -n /verify_loop approved read-only shim targeted tests/` after verify-loop execution delegation removal ? PASS: 4 runs, 145 assertions, 0 failures, 0 errors
- `ruby -Itest test/test_agent_os_v32_static_surface_audit.rb` after verify-loop read-only shim demotion ? PASS: 6 runs, 194 assertions, 0 failures, 0 errors
- `ruby -Itest test/test_aiweb_cli.rb -n /test_github_deploy_help_and_webbuilder_passthrough_surface|deploy/` after deploy guidance demotion ? PASS: 8 runs, 300 assertions, 0 failures, 0 errors
- `ruby -Itest test/test_agent_os_v32_static_surface_audit.rb` after deploy guidance demotion ? PASS: 6 runs, 184 assertions, 0 failures, 0 errors
- `ruby -Itest test/test_agent_os_v32_static_surface_audit.rb -n /test_live_guidance_does_not_reintroduce_approved_only_execution_shortcuts|test_public_product_docs_do_not_market_engine_run_with_manus_claims/` after deploy guidance demotion ? PASS: 2 runs, 88 assertions, 0 failures, 0 errors
- `ruby -Itest test/test_aiweb_cli.rb -n /test_github_deploy_help_and_webbuilder_passthrough_surface/` after deploy guidance demotion ? PASS: 1 run, 34 assertions, 0 failures, 0 errors
