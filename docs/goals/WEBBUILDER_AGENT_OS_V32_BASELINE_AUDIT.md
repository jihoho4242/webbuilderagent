...
- `verify-loop`: converted to a removed legacy command tombstone. The fixed build/preview/QA/visual-critique/repair/agent-run pipeline and its direct execution helpers have been deleted; it no longer delegates to `engine-run`, emits no approval hash, and local work must use `agent` or `engine-run` directly.
